...
def prepare_data(pairs_trained, pairs_tested_or_valed, trim_min_count, generate_nums, copy_nums, tree=False, use_bert=False, auto_transformer=False, bert_pretrain_path='../pretrain/bert-base-chinese'):
    input_lang = Lang()
    output_lang = Lang()
    train_pairs = []
    test_or_val_pairs = ()
    ...
    if use_bert:
        if auto_transformer:
            print("auto_tokenizer", bert_pretrain_path)
            bert_tokenizer = AutoTokenizer.from_pretrained(bert_pretrain_path)
        else:
    ...
    for pairs in pairs_tested_or_valed:
        tmp_pairs = []
        for pair in pairs:
            num_stack = []
            for word in pair[1]:
            ...
            if use_bert:
                input_cell = bert_tokenizer.convert_tokens_to_ids(pair[0])
            tmp_pairs.append((input_cell, len(input_cell), output_cell, len(output_cell),
                            pair[2], pair[3], num_stack))
        print('Number of testind data %d' % (len(tmp_pairs)))
        test_or_val_pairs += (tmp_pairs, )
    if use_bert:
        input_lang = None
    return input_lang, output_lang, train_pairs, test_or_val_pairs, len_bert_token
...
# prepare the batches
def prepare_train_batch(pairs_to_batch, batch_size, shuffle=True):
...
def append_ret(datas, new_ret): # will change data
    for i, data in enumerate(datas):
        for j, data_batch in enumerate(data):
            data[j] += (new_ret[i][j],)
    return datas
...
def prepare_contra_train_batch(train_pairs, batch_size, contra_pair_in, subtree_pos_pair_in, args, neg_sample_num, neg_sample_from_pair_file=False):
    ...
    # neg sample
    order = [p[1] for p in contra_pair]
    for i in range(neg_sample_num):
        if neg_sample_from_pair_file:
            order = [p[2+i] for p in contra_pair]
        else:
            random.shuffle(order)
        neg_pairs = [copy.deepcopy(train_pairs[p]) for p in order]
        ret = append_ret(ret, prepare_train_batch(neg_pairs, batch_size, shuffle=False))
    # subtree_pos
    if args.contra_common_tree_pair:
        pos = 0
        subtree_pos_pair_batches = []
        while pos + batch_size < len(subtree_pos_pair):
            subtree_pos_pair_batches.append(subtree_pos_pair[pos:pos+batch_size])
            pos += batch_size
        subtree_pos_pair_batches.append(subtree_pos_pair[pos:])
        ret.append(subtree_pos_pair_batches)
    return ret # [input_batches, input_lengths, ...] input_batches=[(i, i+, j1-, j2-, ...), (), ], ret_shape = 8 * B * (2+neg_sample) * data_shape
